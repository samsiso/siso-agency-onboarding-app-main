// 🧠 TELEGRAM BOT - AI CONVERSATION ENGINE EXAMPLE
// This shows how to implement the AI-powered conversation intelligence
...
class ConversationEngine {
  ...
  async analyzeMessageIntent(message, userContext) {
    ...
    const intents = {
      // Technical intents
      bug_report: this.calculateIntentScore(messageLower, [
        'bug', 'error', 'broken', 'not working', 'issue', 'problem', 'crash'
      ]),
      feature_request: this.calculateIntentScore(messageLower, [
        'feature', 'add', 'new', 'enhancement', 'improve', 'could you', 'suggestion'
      ]),
      question: this.calculateIntentScore(messageLower, [
        'how', 'what', 'why', 'when', 'where', 'help', '?', 'explain'
      ]),
      
      // Business intents
      financial_query: this.calculateIntentScore(messageLower, [
        'money', 'cost', 'expense', 'budget', 'financial', 'revenue', 'payment'
      ]),
      client_management: this.calculateIntentScore(messageLower, [
        'client', 'customer', 'user', 'onboarding', 'project'
      ]),
      
      // Personal intents
      scheduling: this.calculateIntentScore(messageLower, [
        'schedule', 'meeting', 'appointment', 'calendar', 'time', 'date'
      ]),
      reminder: this.calculateIntentScore(messageLower, [
        'remind', 'reminder', 'later', 'tomorrow', 'next week'
      ]),
      idea_capture: this.calculateIntentScore(messageLower, [
...
// 🧪 Example Usage
async function demonstrateConversationEngine() {
  const engine = new ConversationEngine();
  
  console.log('🧠 AI Conversation Engine Demo\n');
  
  // Simulate user messages
  const testMessages = [
    { userId: 'user123', message: 'Dashboard login is broken' },
    { userId: 'user123', message: 'I want to add a new feature to the agency app' },
    { userId: 'user123', message: 'How much did I spend on software this month?' },
    { userId: 'user123', message: 'Schedule a civic meeting for next week' },
    { userId: 'user123', message: 'Random message with no clear intent' }
  ];
  
  for (const { userId, message } of testMessages) {
    console.log(`👤 User: "${message}"`);
    
    const result = await engine.processMessage(userId, message);
    
    console.log(`�� Intent: ${result.analysis.primaryIntent} (${(result.confidence * 100).toFixed(1)}% confidence)`);
    console.log(`🎯 Repository: ${result.analysis.repository}`);
  ...
  // Show user analytics
  const analytics = engine.getConversationAnalytics('user123');
  console.log('📊 User Analytics:');
  console.log(`Total Messages: ${analytics.totalMessages}`);
  console.log(`Preferred Repositories: ${analytics.preferredRepositories.join(', ')}`);
  console.log(`Top Intents: ${analytics.topIntents.map(([intent, count]) => `${intent} (${count})`).join(', ')}`);
}

// Run demo if this file is executed directly
if (require.main === module) {
  demonstrateConversationEngine();
}
