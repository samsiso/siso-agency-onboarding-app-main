...
const invoke = async <T>(cmd: string, args?: any): Promise<T> => {
  if (!isTauri()) {
    ...
    // Try to connect to running Claudia instance at localhost:1420
    if (cmd === 'execute_claude_code' || cmd === 'continue_claude_code' || cmd === 'resume_claude_code') {
      try {
        const { projectPath, prompt, model } = args || {};
        
        console.log(`Connecting to Claudia for Claude Code execution:`, { projectPath, prompt, model });
        
        // Try to send request to Claudia's API
        const response = await fetch('http://localhost:1420/api/execute', {
          method: 'POST',
          headers: {
...
This is a fallback mock response.`;
    ...
    // Return mock data for other development commands
