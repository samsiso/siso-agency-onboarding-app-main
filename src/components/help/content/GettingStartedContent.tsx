    ...
    >
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-muted-foreground">Follow this step-by-step guide to set up your account and start using the platform</p>
        </div>
      </div>
      
      {/* Welcome Card */}
      <motion.div variants={item}>
        <Card className="bg-gradient-to-br from-black/40 to-black/20 border-gray-800 mb-8">
          <CardHeader className="pb-2">
            <Badge className="self-start mb-2 bg-siso-orange/20 text-siso-orange border-none">New User Guide</Badge>
          ...
          <CardContent>
            <div className="flex flex-col md:flex-row gap-4 mt-2">
              <div className="bg-black/30 p-4 rounded-lg border border-gray-800 flex items-center gap-3 flex-1">
                <div className="h-10 w-10 rounded-full bg-green-500/20 flex items-center justify-center">
                  <CheckCircle2 className="h-5 w-5 text-green-500" />
                </div>
                <div>
                  <p className="text-sm text-gray-400">Your Progress</p>
                  <p className="text-lg font-semibold">33% Complete</p>
                </div>
              </div>
              <div className="bg-black/30 p-4 rounded-lg border border-gray-800 flex items-center gap-3 flex-1">
      ...
      <div className="space-y-6">
        {steps.map((step, index) => (
          <motion.div key={index} variants={item}>
            <Card className={`border-l-4 ${step.completed ? 'border-l-green-500' : 'border-l-gray-600'} bg-black/30 border-t border-r border-b border-gray-800`}>
              <CardContent className="pt-6">
                <div className="flex items-start gap-4">
                  <div className="flex flex-col items-center mr-2">
                    <div className={`h-10 w-10 rounded-full flex items-center justify-center ${step.completed ? 'bg-green-500/20 text-green-500' : 'bg-gray-800 text-gray-400'}`}>
                      {step.completed ? (
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between items-start">
                      <div>
                        <h3 className="text-xl font-semibold flex items-center">
                          {step.title}
                          {step.completed && (
                            <Badge className="ml-2 bg-green-500/20 text-green-500 border-none">Completed</Badge>
                          )}
                        </h3>
                    ...
                    <div className="mt-4">
                      <Button 
                        ...
                        className={step.completed ? "bg-transparent border-green-500 text-green-500 hover:bg-green-500/10" : ""}
                      >
